...
CREATE TABLE IF NOT EXISTS payment_intents (
  ...
  chain_id        text CHECK (chain_id IS NULL OR chain_id IN ('base', 'eth', 'ton', 'sol', 'base_sepolia', 'eth_sepolia')),
  token           text CHECK (token IS NULL OR token IN ('usdt', 'usdc', 'ausd')),
  tx_hash         text,
  from_address    text,
  to_address      text,
  block_number    bigint,
